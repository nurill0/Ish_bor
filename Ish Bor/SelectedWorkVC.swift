...
//  Ish Bor
//
//  Created by Nurillo Domlajonov on 28/03/23.
//

import UIKit
import GoogleMaps

class SelectedWorkVC: BaseController, CLLocationManagerDelegate {
    ...
    lazy var workTitleLbl: UILabel = {
        ...
        lbl.text = "Yani uyga elektr mantaj ishi bor"
        lbl.font = UIFont(name: "Futura-Bold", size: 17)
    ...
    lazy var getWorkJobItem: GetJobWorkItemView = {
        let view = GetJobWorkItemView(icon: "person.fill", username: "Nurillo Domlajonov", job: "Elektrik", rating: "3.9", btnTitle: "Ishni olish")
...
extension SelectedWorkVC{
    ...
    fileprivate func changeFont(){
        if view.frame.height >= 670 && view.frame.height>740 {
            workTitleLbl.font = UIFont(name: "Futura-Bold", size: 19)
            jobTitleLbl.font  = UIFont(name: "Futura-Bold", size: 19)
            getWorkJobItem.height(170)
        }else if view.frame.height>670&&view.frame.height<740{
            getWorkJobItem.height(150)
            workTitleLbl.font = UIFont(name: "Futura-Bold", size: 18)
            jobTitleLbl.font  = UIFont(name: "Futura-Bold", size: 18)
        }
        else{
            workTitleLbl.font = UIFont(name: "Futura-Bold", size: 17)
            jobTitleLbl.font  = UIFont(name: "Futura-Bold", size: 17)
    ...
    fileprivate func workTitleLblConst(){
        itemView.addSubview(workTitleLbl)
        workTitleLbl.top(swipeBtn.bottomAnchor, 5)
        workTitleLbl.left(itemView.leftAnchor, 20)
        workTitleLbl.right(itemView.rightAnchor, -10)
    }
    
    fileprivate func addressLblConst(){
        itemView.addSubview(addressLbl)
        addressLbl.top(workTitleLbl.bottomAnchor, 5)
        addressLbl.left(workTitleLbl.leftAnchor)
        addressLbl.right(workTitleLbl.rightAnchor)
    }
    
    
    fileprivate func priceLblConst(){
        itemView.addSubview(priceLbl)
        priceLbl.top(addressLbl.bottomAnchor, 5)
        priceLbl.left(workTitleLbl.leftAnchor)
        priceLbl.right(workTitleLbl.rightAnchor)
    }
    
    fileprivate func jobTitleLblConst(){
        itemView.addSubview(jobTitleLbl)
        jobTitleLbl.top(priceLbl.bottomAnchor, 10)
        jobTitleLbl.right(workTitleLbl.rightAnchor)
        jobTitleLbl.left(workTitleLbl.leftAnchor)
    }
    ...
    fileprivate func getJobWorkItemViewConst(){
        ...
        getWorkJobItem.right(descriptionLbl.rightAnchor)
        getWorkJobItem.left(descriptionLbl.leftAnchor)
    }
